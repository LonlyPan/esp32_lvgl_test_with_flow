...
typedef struct {
    esp_lcd_panel_t base;                    // 函数接口（屏幕自身的设置相关函数）
    esp_lcd_panel_io_handle_t io;            // 输入输出函数接口（外部调用显示）
    int reset_gpio_num;                      // 硬件复位引脚
    bool reset_level;                        // 复位电平逻辑
    int x_gap;                               // X补偿偏移
    int y_gap;                               // Y补偿偏移
    uint8_t fb_bits_per_pixel;               // 每个像素的位数。例如565 16位  666 24位
    uint8_t madctl_val;                      // 内存数据存取控制指令临时缓存save current value of LCD_CMD_MADCTL register
    uint8_t colmod_val;                      // 颜色格式  RGB，BGR save current value of LCD_CMD_COLMOD register
    const st7796s_lcd_init_cmd_t *init_cmds;  // 自定义初始化cmd，用于兼容不同的初始化配置需求，比如横竖屏等（尽管这些也可以使用其它函数修改）
    uint16_t init_cmds_size;                 // 自定义初始化cmd的数组大小
} st7796s_panel_t;
...
esp_err_t esp_lcd_new_panel_st7796s(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    st7796s = (st7796s_panel_t *)calloc(1, sizeof(st7796s_panel_t));
    ...
    case 16: // RGB565
        st7796s->colmod_val = 0x05;
        st7796s->fb_bits_per_pixel = 16;
        break;
    case 18: // RGB666
        st7796s->colmod_val = 0x06;
        // each color component (R/G/B) should occupy the 6 high bits of a byte, which means 3 full bytes are required for a pixel
        st7796s->fb_bits_per_pixel = 24;
        break;
    case 24: // RGB888
        st7796s->colmod_val = 0x07;
        st7796s->fb_bits_per_pixel = 24;
        break;
    default:
    ...
    if (panel_dev_config->vendor_config) {
        st7796s->init_cmds = ((st7796s_vendor_config_t *)panel_dev_config->vendor_config)->init_cmds;
        st7796s->init_cmds_size = ((st7796s_vendor_config_t *)panel_dev_config->vendor_config)->init_cmds_size;
    }
    ...
    *ret_panel = &(st7796s->base);
    ...
    ESP_LOGI(TAG, "LCD panel create success, version: %d.%d.%d", ESP_LCD_st7796s_VER_MAJOR, ESP_LCD_st7796s_VER_MINOR,
             ESP_LCD_st7796s_VER_PATCH);
...
static const st7796s_lcd_init_cmd_t vendor_specific_init_default[] = {
...
static esp_err_t panel_st7796s_init(esp_lcd_panel_t *panel)
{
    ...
    const st7796s_lcd_init_cmd_t *init_cmds = NULL;
    uint16_t init_cmds_size = 0;
    if (st7796s->init_cmds) {
        init_cmds = st7796s->init_cmds;
        init_cmds_size = st7796s->init_cmds_size;
    } else {
        init_cmds = vendor_specific_init_default;
        init_cmds_size = sizeof(vendor_specific_init_default) / sizeof(st7796s_lcd_init_cmd_t);
    }
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ...
        case LCD_CMD_MADCTL:
            ...
            st7796s->madctl_val = ((uint8_t *)init_cmds[i].data)[0];
            break;
        case LCD_CMD_COLMOD:
            is_cmd_overwritten = true;
            st7796s->colmod_val = ((uint8_t *)init_cmds[i].data)[0];
            break;
        default:
